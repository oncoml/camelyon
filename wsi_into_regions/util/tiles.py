...
def generate_tile_summaries(tile_sum, np_img, display=True, save_summary=False):
  ...
  # np_orig = slide.open_image_np(file_title)
  summary_orig = create_summary_pil_img(np_img, z, row_tile_size, col_tile_size, num_row_tiles, num_col_tiles)
...
def generate_top_tile_summaries(tile_sum, np_img, display=True, save_summary=False, show_top_stats=True,
  ...
  #original_img_path = slide.get_training_image_path(slide_num)
  np_orig = slide.open_image_np('patient_012_node_000.png')
...
def summary_and_tiles(file_title, np_img, large_w, large_h, small_w, small_h, display=False, save_summary=False, save_data=False, save_top_tiles=False):
  """
  Generate tile summary and top tiles for slide.
  Args:
    slide_num: The slide number.
    ...
    save_top_tiles: If True, save top tiles to files.
  """
  tile_sum = score_tiles(file_title, np_img, large_w, large_h, small_w, small_h)
  if save_data:
    save_tile_data(tile_sum)
  generate_tile_summaries(tile_sum, np_img, display=display, save_summary=save_summary)
  # generate_top_tile_summaries(tile_sum, np_img, display=display, save_summary=save_summary)
...
# def summary_and_tiles_2(slide_num, np_img=None, display=True, save_summary=True, save_data=False, save_top_tiles=True):
#   """
#   Generate tile summary and top tiles for slide.
#   Args:
#   Args:
...
#   """
#   #if np_img == None:
#     #np_img = slide.open_image_np('patient_012_node_000.png')
#
#   tile_sum = score_tiles_2(slide_num, np_img)
#   if save_data:
#     save_tile_data(tile_sum)
#   generate_tile_summaries(tile_sum, np_img, display=display, save_summary=save_summary)
#   generate_top_tile_summaries(tile_sum, np_img, display=display, save_summary=save_summary)
...
def tile_to_pil_tile(tile, filename, wsi_directory):
  """
  ...
  s = slide.open_slide(wsi_directory + filename)
...
def tile_to_np_tile(tile, filename, wsi_directory):
  """
  ...
  """
  pil_img = tile_to_pil_tile(tile, filename, wsi_directory)
  np_img = util.pil_to_np_rgb(pil_img)
  return np_img


def save_display_tile(tile, tile_directory, save=True, display=False):
  """
...
def score_tiles(file_title, np_img, large_w, large_h, small_w, small_h, dimensions=None, small_tile_in_tile=False):
  """
  Score all tiles for a slide and return the results in a TileSummary object.
  Args:
    slide_num: The slide number.
...
#      np_img = slide.open_image_np('patient_012_node_000.png')
  ...
  tile_sum = TileSummary(file_title=file_title,
                       slide_num=None,
                       orig_w=o_w,
                       orig_h=o_h,
                       orig_tile_w=COL_TILE_SIZE,
  ...
  for t in tile_indices:
      ...
      np_scaled_tile = np_tile if small_tile_in_tile else None
      tile = Tile(tile_sum, file_title, None, np_scaled_tile, count, r, c, r_s, r_e, c_s, c_e, o_r_s, o_r_e, o_c_s,
                  o_c_e, t_p, color_factor, s_and_v_factor, quantity_factor, score)
      
      if amount == TissueQuantity.HIGH or amount == TissueQuantity.MEDIUM:
         tile_sum.tiles.append(tile)
...
class TileSummary:
  ...
  def __init__(self, file_title, slide_num, orig_w, orig_h, orig_tile_w, orig_tile_h, scaled_w, scaled_h, scaled_tile_w, scaled_tile_h, tissue_percentage, num_col_tiles, num_row_tiles):
...
class Tile:
  ...
  def __init__(self, tile_summary, file_title, slide_num, np_scaled_tile, tile_num, r, c, r_s, r_e, c_s, c_e, o_r_s, o_r_e, o_c_s,
               o_c_e, t_p, color_factor, s_and_v_factor, quantity_factor, score):
